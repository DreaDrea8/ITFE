...
import { instanceOfErrnoException } from '@src/commons/InstanceOfNodeError'
...
export class FileSystemService {
  ...
  // TODO: Persist file in external service
    ...
    try {
      ...
      if (instanceOfErrnoException(error)){
        if (error.code === 'EEXIST'){
          this.loggerService.error(`Create directory - Already exists ${directoryPath}, ${error.name}: ${error.message}`, error)
        } else if (error.code === 'ENOENT'){
          this.loggerService.error(`Create directory - No such directory ${directoryPath}, ${error.name}: ${error.message}`, error)
        } else {
          this.loggerService.error(`Create directory ${directoryPath}, ${error.name}: ${error.message}`, error)
        }
      } else if(error instanceof Error) {
        this.loggerService.error(`Create directory ${directoryPath}, ${error.name}: ${error.message}`, error)
      } else {
        this.loggerService.error(`Create directory ${directoryPath}, ${JSON.stringify(error)}`)
      }
    }
  }
    ...
    try {
      ...
      if (instanceOfErrnoException(error)){
        if(error.code === 'ENOENT') {
          this.loggerService.error(`List directory - No such directory ${directoryPath}, ${error.name}: ${error.message}`, error)
        } else {
          this.loggerService.error(`List directory ${directoryPath}, ${error.name}: ${error.message}`, error)
        }
      } else if(error instanceof Error) {
        this.loggerService.error(`List directory ${directoryPath}, ${error.name}: ${error.message}`, error)
      } else {
        this.loggerService.error(`List directory  ${directoryPath}, ${JSON.stringify(error)}`)
      }
    }
  }
  ...
  public deleteDirectory = async (directoryRoot:string, directoryLocation:string, directoryName: string, options?: RmDirOptions): Promise<void> => {
    ...
    try {
      ...
      if (instanceOfErrnoException(error)){
        if ( error.code === 'ENOENT'){
          this.loggerService.error(`Delete directory - No such directory, ${directoryPath}, ${error.name}: ${error.message}`, error)
        } else {
          this.loggerService.error(`Delete directory, ${directoryPath}, ${error.name}: ${error.message}`, error)
        }
      } else if(error instanceof Error) {
        this.loggerService.error(`Delete directory, ${directoryPath}, ${error.name}: ${error.message}`, error)
      } else {
        this.loggerService.error(`Delete directory, ${directoryPath}, ${JSON.stringify(error)}`)
      }
    }
  }
  ...
  public getFileInfo = async (directoryRoot:string, fileLocation:string, fileName: string, options?:StatOptions): Promise<Stats|BigIntStats|void>  => {
    ...
    try {
      ...
      if (instanceOfErrnoException(error)){
        if (error.code === 'ENOENT'){
          this.loggerService.error(`Get file info - No such file ${filePath}, ${error.name}: ${error.message}`, error)
        } else {
          this.loggerService.error(`Get file info ${filePath}, ${error.name}: ${error.message}`, error)
        }
      } else if(error instanceof Error) {
        this.loggerService.error(`Get file info ${filePath}, ${error.name}: ${error.message}`, error)
      } else {
        this.loggerService.error(`Get file info ${filePath}, ${JSON.stringify(error)}`)
      }
    }
  }
    ...
    try {
      ...
      if (instanceOfErrnoException(error)){
        if (error.code === 'ENOENT'){
          ...
          this.loggerService.error(`Update file content ${filePath}, ${error.name}: ${error.message}`, error)
        }
      } else if(error instanceof Error) {
        this.loggerService.error(`Update file content ${filePath}, ${error.name}: ${error.message}`, error)
      } else {
        this.loggerService.error(`Update file content ${filePath}, ${JSON.stringify(error)}`)
      }
    }
  }
  ...
  public renameFile = async (directoryRoot:string, oldFileLocation:string, oldFileName: string, newFileLocation: string, newFileName: string): Promise<void>  => {
    ...
    try {
      ...
      if (instanceOfErrnoException(error)){
        if (error.code === 'EEXIST'){
          this.loggerService.error(`Rename file - Already exists ${newFilePath}, ${error.name}: ${error.message}`, error)
        } else if (error.code === 'ENOENT'){
          ...
          this.loggerService.error(`Rename file ${newFilePath}, ${error.name}: ${error.message}`, error)
        }
      } else if(error instanceof Error) {
        this.loggerService.error(`Rename file ${newFilePath}, ${error.name}: ${error.message}`, error)
      } else {
        this.loggerService.error(`Rename file ${newFilePath}, ${JSON.stringify(error)}`)
      }
    }
  }
  ...
  public cpFile = async (directoryRoot:string, oldFileLocation:string, oldFileName: string, newFileLocation: string, newFileName: string): Promise<void>  => {
    ...
    try {
      ...
      if (instanceOfErrnoException(error)){
        if (error.code === 'EEXIST'){
          this.loggerService.error(`Copy file - Already exists ${newFilePath}, ${error.name}: ${error.message}`, error)
        } else if (error.code === 'ENOENT'){
          const files:string[] = await this.listDirectory(directoryRoot, oldFileLocation) ?? []
          for(const fileName of files){
            if (fileName === oldFileName && this.guard){
          ...
          this.loggerService.error(`Copy file ${newFilePath}, ${error.name}: ${error.message}`, error)
        }
      } else if(error instanceof Error) {
        this.loggerService.error(`Copy file ${newFilePath}, ${error.name}: ${error.message}`, error)
      } else {
        this.loggerService.error(`Copy file ${newFilePath}, ${JSON.stringify(error)}`)
      }
    }
  }
  ...
  public deleteFile = async (directoryRoot:string, directoryLocation:string, directoryName: string): Promise<void> => {
    ...
    try {
      ...
      if (instanceOfErrnoException(error)){
        if (error.code === 'ENOENT'){
          this.loggerService.error(`Delete file - No such file, ${directoryPath}, ${error.name}: ${error.message}`, error)
        } else {
          this.loggerService.error(`Delete file, ${directoryPath}, ${error.name}: ${error.message}`, error)
        }
      } else if(error instanceof Error) {
        this.loggerService.error(`Delete file, ${directoryPath}, ${error.name}: ${error.message}`, error)
      } else {
        this.loggerService.error(`Delete file, ${directoryPath}, ${JSON.stringify(error)}`)
      }
    }
  }
}
